...
export default function FrequencySettingsForm({
  initialPreferences,
  onSubmit,
}: FrequencySettingsFormProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [selectedFrequency, setSelectedFrequency] = useState(
    initialPreferences.defaultFrequency
  );
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Ustaw domyślną częstotliwość sprawdzania cen dla nowych ofert
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form id="frequency-form" onSubmit={handleSubmit(handleFormSubmit)}>
          <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Zmiany dotyczą tylko nowo dodawanych ofert
            </p>
          </div>
        </form>
      </CardContent>
